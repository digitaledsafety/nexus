...
contract ExhibitVault is ERC721Holder, ERC1155Holder, ReentrancyGuard, AccessControl {
    ...
    /**
     * @dev Batch extend exhibition duration for active ERC721 tokens.
     */
    function batchExtendExhibition721(
        address[] calldata nftContracts,
        uint256[] calldata tokenIds,
        uint256[] calldata durations
    ) external nonReentrant {
        uint256 len = nftContracts.length;
        require(len == tokenIds.length && len == durations.length, "Mismatched arrays");
    ...
    /**
     * @dev Batch extend exhibition duration for active ERC1155 tokens.
     */
    function batchExtendExhibition1155(
        address[] calldata nftContracts,
        uint256[] calldata tokenIds,
        uint256[] calldata durations
    ) external nonReentrant {
        ...
        for (uint256 i = 0; i < len; ) {
            ...
            emit Exhibited1155(
                ...
                tokenId,
                msg.sender,
                balances1155[nftContract][tokenId][msg.sender],
                location,
                expiry1155[nftContract][tokenId][msg.sender]
            );
