...
contract NFTMarketplace is ReentrancyGuard, AccessControl {
    ...
    struct Offer {
        ...
        uint256 expiry;    // Expiry timestamp (0 for no expiry)
    }
    ...
    /**
     * @notice Create an offer for an NFT with an expiry timestamp
     */
    function createOffer(address nftContract, uint256 tokenId, uint256 amount, uint256 price, uint256 expiry) external {
        _createOffer(nftContract, tokenId, amount, price, expiry);
    }

    function _createOffer(address nftContract, uint256 tokenId, uint256 amount, uint256 price, uint256 expiry) internal nonReentrant {
    ...
    /**
     * @notice Update an existing offer with an expiry timestamp
     */
    function updateOffer(address nftContract, uint256 tokenId, uint256 newAmount, uint256 newPrice, uint256 newExpiry) external {
        _updateOffer(nftContract, tokenId, newAmount, newPrice, newExpiry);
    }

    function _updateOffer(address nftContract, uint256 tokenId, uint256 newAmount, uint256 newPrice, uint256 newExpiry) internal nonReentrant {
