...
contract NFTMarketplace is ReentrancyGuard, AccessControl {
    ...
    function _acceptOffer(address nftContract, uint256 tokenId, address buyer) internal {
        ...
        // Also clear any listing the seller might have for this token
        delete listings[nftContract][tokenId][msg.sender];

        _distributeProceeds(nftContract, tokenId, msg.sender, offer.price);
    ...
    function cancelListing(address nftContract, uint256 tokenId) external nonReentrant {
        require(listings[nftContract][tokenId][msg.sender].price > 0, "No listing exists");
        delete listings[nftContract][tokenId][msg.sender];
    ...
    function buyFromListing(address nftContract, uint256 tokenId, address seller) external nonReentrant {
        ...
        // CEI: Clear the listing
        delete listings[nftContract][tokenId][seller];

        // Also clear any offer the buyer might have for this token
    ...
    /**
     * @dev Internal helper to handle protocol fees and EIP-2981 royalties.
     */
