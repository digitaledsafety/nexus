...
contract BragNFT is ERC721URIStorage, AccessControl, ReentrancyGuard, Pausable, IERC2981, IERC6454 {
    ...
    /**
     * @dev Process internal donation logic without ETH transfer to treasury.
     */
    ...
    /**
     * @dev Internal donation logic. Records a permanent tax record, mints NFT and transfers ETH to treasury.
     */
    ...
    function batchDonate(string[] calldata messages, string[] calldata tokenURIs) external payable nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < count; ) {
            uint256 amount = (i == count - 1) ? (ethPerNft + remainder) : ethPerNft;
            _processDonation(msg.sender, messages[i], tokenURIs[i], false, amount);
            unchecked { i++; }
        }
    ...
    /**
     * @dev Mint multiple BragNFTs to specified recipients by donating ETH in batch.
     */
    function batchDonateTo(address[] calldata recipients, string[] calldata messages, string[] calldata tokenURIs) external payable nonReentrant whenNotPaused {
        require(recipients.length == messages.length && messages.length == tokenURIs.length, "Mismatched arrays");
        ...
        for (uint256 i = 0; i < count; ) {
            uint256 amount = (i == count - 1) ? (ethPerNft + remainder) : ethPerNft;
            _processDonation(recipients[i], messages[i], tokenURIs[i], false, amount);
            unchecked { i++; }
        }
