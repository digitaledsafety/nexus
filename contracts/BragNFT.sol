...
contract BragNFT is ERC721URIStorage, AccessControl, ReentrancyGuard, Pausable, IERC2981, IERC6454 {
     ...
     */
    function batchUpdateOnChainMedia(uint256[] calldata tokenIds, string[] calldata mediaUris) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(tokenIds.length == mediaUris.length, "Mismatched arrays");
        for (uint256 i = 0; i < tokenIds.length; ) {
            _requireOwned(tokenIds[i]);
            onChainMedia[tokenIds[i]] = mediaUris[i];
            unchecked { i++; }
        }
    }
