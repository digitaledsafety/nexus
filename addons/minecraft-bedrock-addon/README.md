...
## Troubleshooting Remote Server & Connection Issues

### 1. Distinguishing System SSH Logs from WebSocket Logs
If you inspect system logs on a remote virtual machine (VM) hosting your server, you may see entries like:
```text
pam_unix(sshd:auth): authentication failure; logname= uid=0 euid=0 tty=ssh ruser= rhost=...
```
* **Important**: These log lines are generated by Linux OpenSSH (`sshd`) on port 22 when automated internet scanners attempt unauthorized SSH logins.
* They **do not** indicate an authentication failure for your Node.js WebSocket bridge (`scripts/nft-bridge.js`) or Minecraft Bedrock.

### 2. WebSocket Connection Architecture
* In Minecraft Bedrock Edition, the game engine (client or Bedrock Dedicated Server) acts as the WebSocket client and **initiates an outbound connection** to the bridge server (`nft-bridge.js` listening on port `9001`).
* Command executed in-game or by script:
  ```text
  /connect ws://<YOUR_VM_PUBLIC_IP>:9001
  ```

### 3. Resolving Connection Failures (`/connect` Failure)
If Minecraft cannot connect to the WebSocket bridge on a remote VM:
1. **Update `WS_URL` in Addon Configuration**:
   - By default, `addons/minecraft-bedrock-addon/development_behavior_packs/behavior_pack_sample/scripts/config.js` is set to `ws://127.0.0.1:9001`.
   - Update `config.js` or set environment variable `WS_URL=ws://<YOUR_VM_PUBLIC_IP>:9001` before running `npm run env:init`.
2. **Open Port 9001 on the Remote Host**:
   - Open TCP port `9001` in your cloud provider's Firewall / Security Group rules (e.g., AWS Security Group, Azure Network Security Group, GCP Firewall).
   - If using Linux `ufw`, allow port 9001:
     ```bash
     sudo ufw allow 9001/tcp
     ```
3. **Verify Bridge Server Logs**:
   - The NFT bridge includes connection logging (`[WS] Connection established from <IP>`). Check stdout of `node scripts/nft-bridge.js` or `npm run bridge` to confirm incoming connections.
