...
describe("Marketplace Batch Operations", async function () {
  ...
  it("Should batch update listings and private listings", async function () {
    ...
    let l1 = await marketplace.read.listings([bragNFT.address, 0n, seller.account.address]);
    let l2 = await marketplace.read.listings([bragNFT.address, 1n, seller.account.address]);
    ...
    l1 = await marketplace.read.listings([bragNFT.address, 0n, seller.account.address]);
    l2 = await marketplace.read.listings([bragNFT.address, 1n, seller.account.address]);
    assert.equal(l1[1], parseEther("3"));
    assert.equal(l1[3], getAddress(buyer.account.address));
    assert.equal(l2[1], parseEther("4"));
    assert.equal(l2[3], getAddress(buyer.account.address));
  });
  ...
  it("Should revert batch update listings if array lengths mismatch", async function () {
    ...
    await assert.rejects(
      marketplace.write.batchUpdatePrivateListings([[bragNFT.address], [0n, 1n], [1n], [parseEther("1")], [seller.account.address]], { account: seller.account }),
      /Mismatched arrays/
    );
  });
});
